...
호텔 언급이 4회를 넘으면 자동으로 재작성한다.

**한국어와 영어는 주제 풀이 다르다.** 검색 의도가 다르기 때문이다.

| | 한국어 (`config/topics.json`) | 영어 (`config/topics_en.json`) |
|---|---|---|
| 독자 | 이미 제주행을 정한 국내 여행자 | 한국 여행을 계획 중인 외국인 |
| 검색 시점 | 출발 직전, 현지에서 | 몇 달 전, 항공권 알아보며 |
| 검색 예시 | 성산 맛집, 광치기 물때, 주차 | do I need a car in Jeju, Jeju bus 201, best time to visit |
| 주제 수 | 60 | 40 |
| 예약 유도 | 네이버 예약 (직접예약) | Agoda / Trip.com (리뷰 축적) |

영문 글은 한국인에게는 설명이 필요 없는 것(버스 요금 결제, 편의점 활용,
체크인 절차, 일회용품 미제공)을 반드시 설명하도록 프롬프트에 걸려 있다.
...
│  ├─ topics.json     한국어 주제 60개. 소진되면 Claude가 20개씩 보충
│  └─ topics_en.json  영어 주제 40개 (외국인 검색 기준). 소진 시 동일
├─ assets/
